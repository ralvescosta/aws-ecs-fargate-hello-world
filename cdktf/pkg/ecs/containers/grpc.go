...
func NewGrpcContainer(stack *stack.MyStack) {
	logGroupName := fmt.Sprintf("ecs/%v-grpc", stack.Cfgs.AppName)
	...
	containerName := fmt.Sprintf("%v-fna-grpc", stack.Cfgs.AppName)

	containerDefinitions := `
	[
		{
			"cpu": 256,
			"image": "rafaelbodao/ecs-grpc:latest",
			"name": "<<CONTAINER_NAME>>",
	...
	ecsTaskDefinitionName := fmt.Sprintf("%v-ecs-grpc-td", stack.Cfgs.AppName)
	...
	ecsTaskDefinitionSecGroupName := fmt.Sprintf("%v-ecs-grpc-sec-group", stack.Cfgs.AppName)
	...
	ecsServiceName := fmt.Sprintf("%v-ecs-grpc-svc", stack.Cfgs.AppName)
